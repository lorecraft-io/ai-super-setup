...
W4W_EOF
    ...
    # --- /bullets skill ---
    # Crushes a prose blob into the shortest scannable bullets. Sibling to
    # /concise: always bulletizes (where /concise sometimes leaves prose).
    # Single-file, inlined like /w4w — no network dependency.
    BULLETS_DIR="$HOME/.claude/skills/bullets"
    mkdir -p "$BULLETS_DIR"
    cat > "$BULLETS_DIR/SKILL.md" << 'BULLETS_EOF'
---
name: bullets
...
BULLETS_EOF
    success "Bullets skill (/bullets) installed"
...
run_self_test() {
    ...
    if [ -f "$HOME/.claude/skills/bullets/SKILL.md" ]; then
        success "TEST: Bullets skill (/bullets) installed"
        TEST_PASS=$((TEST_PASS + 1))
    else
        soft_fail "TEST: Bullets skill (/bullets) not found"
        TEST_FAIL=$((TEST_FAIL + 1))
    fi
...
print_summary() {
    ...
    echo "    /bullets   — crush a paragraph into the shortest scannable bullets"
    echo ""
